# Use an official Node.js runtime as a parent image
FROM node:16.17-alpine


# Set the working directory to /app
WORKDIR /app

# Copy package.json and package-lock.json to the container
COPY package*.json ./

# Install dependencies
RUN npm install

# Copy the rest of the application code to the container
COPY . .
ENV NEXT_PUBLIC_STRAPI_TOKEN 
ENV NEXT_PUBLIC_STRAPI_ENDPOINT https://strapi.klaim.ai/api
ENV NEXT_PUBLIC_STRAPI_STATIC_ENDPOINT https://strapi.klaim.ai
ENV NEXT_PUBLIC_POSTHOG_KEY phc_dH7RNDqPVDoQyXsVXBdEjaxGOlqs7tpj8nVIMG8XraF
ENV NEXT_PUBLIC_POSTHOG_HOST https://eu.i.posthog.com

RUN npm run build

EXPOSE 3000

CMD ["npm", "run", "start"]
