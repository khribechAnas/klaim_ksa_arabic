...
export function CaseStudyPosts({ searchParams }: any) {
  ...
  const loadPosts = (search: string, page: string, limit: string) => {
    ...
    const searchParameters = search
      ...
      : "";
    fetch(
      `${process.env.NEXT_PUBLIC_STRAPI_ENDPOINT}/case-studies?populate=*&pagination[page]=${page}&pagination[pageSize]=${limit}&sort[0]=publishedOn:desc${searchParameters}`,
      {
        headers: {
      ...
      response.json().then((studiesJson) => {
        setCaseStudies(studiesJson.data);
        setPagination(studiesJson.meta.pagination);
        setLoading(false);
      });
    }).catch((e) => {
        setLoading(false);
      ...
      <div className="flex items-center justify-center mb-[7.625rem]">
        <Pagination
          ...
          onChange={(page: number) =>
            loadPosts(searchParams.search, page.toString(), "12")
          }
        />
      </div>
    </section>
