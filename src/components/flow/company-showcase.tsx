...
export function CompanyShowcase() {
  const { companyShowcaseFlow } = siteConfig;
  return (
    <section
      id="company"
      className="flex flex-col items-center justify-center gap-10 py-10 pt-10 w-full relative px-6"
    >
      <p className="text-muted-foreground font-medium">Award Winning Company</p>
      <div className="grid w-full max-w-7xl grid-cols-2 md:grid-cols-4 overflow-hidden border-y border-border items-center justify-center z-20">
        {companyShowcaseFlow.companyLogos.map((logo) => (
          <div
            ...
            key={logo.id}
          >
            <div className="transition-all duration-200 [cubic-bezier(0.165, 0.84, 0.44, 1)] translate-y-0  duration-300 flex items-center justify-center w-full h-full">
              {logo.logo}
            </div>
