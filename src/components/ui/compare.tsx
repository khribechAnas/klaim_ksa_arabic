...
export const Compare = ({
  ...
  return (
    <div
      ...
      {/* First Content/Image (Left side) */}
      <div className="overflow-hidden w-full h-full relative z-20 pointer-events-none">
        <AnimatePresence initial={false}>
          {firstContent ? (
            <motion.div
              className="absolute inset-0 z-20 w-full h-full select-none overflow-hidden"
              style={{
                clipPath: `inset(0 ${100 - sliderXPercent}% 0 0)`,
              }}
              transition={{duration: 0}}>
              <div className="absolute inset-0 z-20 w-full h-full">{firstContent}</div>
            </motion.div>
          ) : firstImage ? (
            <motion.div
      ...
      {/* Second Content/Image (Right side) */}
      <AnimatePresence initial={false}>
        {secondContent ? (
          <div className="absolute top-0 left-0 z-[19] w-full h-full select-none">{secondContent}</div>
        ) : secondImage ? (
          <motion.img
