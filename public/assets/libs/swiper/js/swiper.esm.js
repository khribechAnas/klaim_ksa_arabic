...
const Virtual = {
  ...
  renderSlide(slide, index) {
    ...
    const $slideEl = params.renderSlide
      ...
      : $(
          `<div className="${swiper.params.slideClass}" data-swiper-slide-index="${index}">${slide}</div>`
        );
...
const Scrollbar = {
  ...
  init() {
    ...
    if ($dragEl.length === 0) {
      $dragEl = $(
        `<div className="${swiper.params.scrollbar.dragClass}"></div>`
      );
      $el.append($dragEl);
    }
...
var a11y$1 = {
  ...
  create() {
    ...
    Utils.extend(swiper, {
      a11y: {
        liveRegion: $(
          `<span className="${swiper.params.a11y.notificationClass}" aria-live="assertive" aria-atomic="true"></span>`
        ),
      },
    });
